    ...
    if weight == "" :
        for modAndPdf in c.mods_and_pdfs :
            for ht in c.htbins :
                ...
                outfile = r.TFile("limit_format_normalized_output/cvOnly/%s_%s.root"%(modAndPdf[0],ht[0]),"RECREATE") 
            ...
            for pdfSet in modAndPdf[1] :
                for iweight in range(c.nPdfDict[pdfSet]) :
                	...
                	for ht in c.htbins :
            		     ...
            		     rtfile = "%s/%s_calo_ge2_%s/%s_plots.root"%(c.version,ht[0],weight,modAndPdf[0])
            		     outfile = r.TFile("limit_format_normalized_output/cvOnly/%s_%s_%s_normalized.root"%(modAndPdf[0],ht[0],pdfSet),"UPDATE") 
                	     if "T2cc" in modAndPdf[0] :
            		     ...
            		     for key in befkeylist :
            		         if key.ReadObj().GetName() == "nEvents_%s_%s"%(pdfSet,iweight):
            		             ...
            		             print "Original Before Hist @ (%d,%d) = %s"%(2,8,m0_m12_mChi_noweight.GetBinContent(2,8))
            		             m0_m12_mChi_noweight.Multiply(scaleHist)
            		             print "Scaled Before Hist @ (%d,%d) = %s"%(2,8,m0_m12_mChi_noweight.GetBinContent(2,8))
            		             outfile.cd("smsScan_before")
            		             m0_m12_mChi_noweight.Write("",r.TObject.kOverwrite)
            		     afdir = infile.GetDirectory(ht[1])
            		     afkeylist = afdir.GetListOfKeys()
